...
class HardenedHttpClient:
    def __init__(self, *, timeout_seconds: float = 10.0, max_retries: int = 1, retry_backoff_seconds: float = 0.5):
    ...
    async def post_json(
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, http2=True) as client:
            ...
            if last_exc:
                raise last_exc
            # if no exception but still retryable status, return last response
