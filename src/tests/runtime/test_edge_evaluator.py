...
def test_edge_evaluator_returns_true_for_valid_condition():
    ...
    assert EdgeEvaluator.is_true("validation_status == 'VALID' && confidence >= 0.85", context)


def test_edge_evaluator_returns_false_for_invalid_condition():
    context = {"validation_status": "INVALID", "confidence": 0.5}
    assert not EdgeEvaluator.is_true("validation_status == 'VALID' && confidence >= 0.85", context)
...
def test_edge_evaluator_handles_or_operator():
    ...
    assert EdgeEvaluator.is_true("status == 'ERROR' || status == 'FAILED'", context)
    assert EdgeEvaluator.is_true("status == 'SUCCESS' || status == 'ERROR'", context)
    assert not EdgeEvaluator.is_true("status == 'SUCCESS' || status == 'PENDING'", context)


def test_edge_evaluator_raises_on_invalid_syntax():
    context = {"value": 10}
    with pytest.raises(DomainValidationException, match="edge_evaluation_error"):
