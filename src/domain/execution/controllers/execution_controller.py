...
class ExecutionController:
    ...
    def _bind_routes(self) -> None:
        ...
        r("/flow-runs", self.create_flow_run, methods=["POST"], response_model=FlowRun, status_code=status.HTTP_201_CREATED, deprecated=True, responses=self._resp405())
        r("/tool-runs", self.create_tool_run, methods=["POST"], response_model=ToolRun, status_code=status.HTTP_201_CREATED, deprecated=True, responses=self._resp405())
        r("/tool-runs/{tool_run_id}:execute", self.execute_tool_run, methods=["POST"], response_model=dict, deprecated=True, responses=self._resp405())
        r("/flow-runs/{flow_run_id}", self.get_flow_run, methods=["GET"], response_model=FlowRun, deprecated=True, responses=self._resp405())
        r("/flow-runs/{flow_run_id}/graph-state", self.get_graph_state, methods=["GET"], response_model=GraphState, deprecated=True, responses=self._resp405())
        r("/node-runs", self.list_node_runs, methods=["GET"], response_model=list[NodeRun], deprecated=True, responses=self._resp405())
        r("/agent-runs", self.list_agent_runs, methods=["GET"], response_model=list[AgentRun], deprecated=True, responses=self._resp405())
    ...
    async def get_flow_run(self, flow_run_id: str, _: AuthContext = Depends(get_auth_context)) -> FlowRun:
        raise MethodNotAllowedPlaceholderException()

    async def get_graph_state(self, flow_run_id: str, _: AuthContext = Depends(get_auth_context)) -> GraphState:
        raise MethodNotAllowedPlaceholderException()
