...
class IntentDetectionNode(NodeExecutor):
    ...
    async def execute(
        ...
        model_alias = llm_cfg.get("model_alias")
        ...
        with self.tracer.observe(
            as_type="chain",
            name="domain.execution.nodes.intent_detection.resolve_prompt",
            input={"node_id": str(node_uuid)},
        ) as chain_handle:
        ...
        llm_request = LLMRequest(
            ...
            temperature=llm_cfg.get("temperature") if (llm_cfg and "temperature" in llm_cfg) else llm_policy.get("temperature"),
        ...
        with self.tracer.observe(
            as_type="chain",
            name="domain.execution.nodes.intent_detection.execute_llm",
