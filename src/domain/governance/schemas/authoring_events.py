from enum import StrEnum


class AuthoringEventType(StrEnum):
    AGENT_CREATED = "AGENT_CREATED"
    AGENT_VERSION_CREATED = "AGENT_VERSION_CREATED"
    NODE_AGENT_BINDING_CREATED = "NODE_AGENT_BINDING_CREATED"
    TOOL_IMPORTED = "TOOL_IMPORTED"
    TOOL_CONFIG_CREATED = "TOOL_CONFIG_CREATED"
    TOOL_CONFIG_PUBLISHED = "TOOL_CONFIG_PUBLISHED"
    TOOL_CONFIG_DEPRECATED = "TOOL_CONFIG_DEPRECATED"
    TOOL_CONFIG_DISABLED = "TOOL_CONFIG_DISABLED"
    AGENT_VERSION_TOOL_BINDING_CREATED = "AGENT_VERSION_TOOL_BINDING_CREATED"
    VERSION_PUBLISHED = "VERSION_PUBLISHED"
    VERSION_ACTIVATED = "VERSION_ACTIVATED"
    VERSION_ROLLED_BACK = "VERSION_ROLLED_BACK"
    RAG_CONFIG_CREATED = "RAG_CONFIG_CREATED"
    RAG_CONFIG_VALIDATED = "RAG_CONFIG_VALIDATED"
    RAG_CONFIG_PUBLISHED = "RAG_CONFIG_PUBLISHED"
    RAG_CONFIG_DEPRECATED = "RAG_CONFIG_DEPRECATED"
    RAG_CONFIG_DISABLED = "RAG_CONFIG_DISABLED"
    RAG_CHUNKING_RULE_CREATED = "RAG_CHUNKING_RULE_CREATED"
    RAG_CHUNKING_RULE_UPDATED = "RAG_CHUNKING_RULE_UPDATED"
    TENANT_TOKEN_ISSUED = "TENANT_TOKEN_ISSUED"


class ChangeType(StrEnum):
    CREATE = "CREATE"
    UPDATE = "UPDATE"
    DELETE = "DELETE"


class ResourceType(StrEnum):
    RAG_CONFIG = "rag_config"
    RAG_CHUNKING_RULE = "rag_chunking_rule"
    AGENT = "agent"
    FLOW = "flow"
    TOOL = "tool"
