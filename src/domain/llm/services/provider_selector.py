...
class LLMProviderSelector:
    ...
    async def select(
        ...
        config = await self.provider_repository.get_active_config(tenant_id=tenant_id, provider=provider)
        if config is None:
            raise DomainValidationException(message="llm_provider_not_active")

        mapping = await self.model_mapping_repository.get_active_mapping(
